...
function Header() {
    ...
    const { username, isAuthenticated, isStaff } = useSelector((state: RootState) => state.users);
                    ...
                    <Navbar.Collapse id="navbar-desktop" className="justify-content-end d-none d-lg-flex">
                        <Nav className="flex-row gap-3 align-items-center">
                            ...
                            {isAuthenticated ? (
                                <>
                                    {isStaff && (
                                        <Nav.Link as={Link} to="/moderatecomponent" className="logo-button">Редактирование компонентов</Nav.Link>
                                    )}
